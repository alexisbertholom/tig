...
--bdiff [<rev>]::
	...
	the current branch is used. See the "BASE DIFF" section.
...
git-range-diff(1) does. Without <rev>, HEAD is compared with its upstream,
which tells what a rewrite did to the branch since it was last pushed. '--bdiff-base' compares from another commit instead,
