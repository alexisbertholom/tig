...
--bdiff [<rev>]::
	...
	when the branch tracks nothing. The comparison marks the commits of
	the main view, so it takes no subcommand. See the "BASE DIFF"
	section.
...
A merge commit is compared by the diff its merge left over, which is empty
whenever it went through without a conflict to resolve, so renaming one leaves
nothing of it to recognise. It is then paired by the commit it was made on,
whatever the branch it brings in has become.
...
[from]::
	...
	Both ends of a move carry the same number, as in '[move #1]' and
	'[from #1]', so that they name each other when several commits moved.
