...
The `&` search asks git for the commits whose patch adds or removes a matching
line, and for those whose message matches; each of them is prefixed with a blue
`[match]` in the main view. Only the revisions and paths the view lists are
looked at, so a filtered view is searched no further than what it shows.
