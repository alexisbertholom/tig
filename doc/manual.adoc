...
|&	|Search what the listed commits carry, and not only what the main view
	 shows of them. Also prompts for regexp.
|=============================================================================

A `/` search matches what the view draws and nothing else: the ID column shows
the first characters of a hash, and it is those which are searched, so a
pattern found only in the rest of the hash is no match. Use `:<hash>` to jump
to a commit by an ID which is not on screen.

The `&` search looks for the pattern the three ways the diff of a commit shows
it: in the patch, where git reports the commits adding or removing a matching
line; in the message; and in the names of the files the commit edits. Each
commit which carries it is prefixed with a blue `[match]` in the main view.
The file names can be left out of the search with the 'search-content-paths'
option. Only the revisions and paths the view lists are looked at, so a
filtered view is searched no further than what it shows.
Refreshing the main view searches again, marking the commits which have turned
up since without dropping the markers already there.

The pattern is kept as the search regexp too: `n` then steps from one marked
commit to the next, and, in the diff of any of them, from one match to the
next. An empty pattern drops the markers.
...
|:goto <rev>		|Jump to a specific revision, e.g. `:goto %(commit)^2`
			 ...
			 branch `some/branch`.
|:bdiff [<rev>]		|Compare the history of HEAD with the one of <rev>,
			 or with its upstream, the way `--bdiff` does.
|:&<regexp>		|Mark the commits whose contents match `<regexp>`,
			 the way `&` does.
