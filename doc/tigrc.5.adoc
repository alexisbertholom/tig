...
'diff-highlight' (mixed)::
	...
	value is used as the path. The special value `internal` selects tig's
	built-in word refinement, which needs no external program: each block of
	removed and added lines is diffed word by word, and the words that changed
	are highlighted (a pure insertion or deletion is highlighted whole). When
	this option is in effect, highlighted regions are governed by
	`color diff-add-highlight` and `color diff-del-highlight`.

'diff-indicator' (bool)::

	Show +/- signs in the diff view. On by default.

'diff-stat-group' (bool)::

	Lay the diffstat out as a tree, to shorten the paths. Each directory is
	printed once as a row of its own (colored with `color diff-stat-header`)
	and elided from the files below it. A directory holding a single entry is
	folded into it, `src/lib/parse.c` reading as one row, as far as the view
	width allows. Renames keep the `{old => new}` form git prints them in, so
	a directory moved wholesale is a single node. The tree is drawn with the
	characters of the 'line-graphics' option, ASCII unless it is UTF-8 or the
	locale is. Off by default.
...
'search-content-paths' (bool)::

	Whether the content search bound to '&' also looks for its pattern in
	the names of the files each commit edits, as the diff of a commit
	shows them. On by default; with it off, only the patch and the message
	are searched. Changing it searches again, since it changes which
	commits match.
...
|load-more               |Load all commits past the -n limit
|maximize                |Maximize the current view
|view-close              |Close the current view
|view-close-no-quit      |Close the current view without quitting
|view-close-force        |Close the current view without first restoring the split
...
|diff-apply-chunk        |Apply the diff chunk to the working tree
|diff-revert-chunk       |Revert the diff chunk from the working tree
|=============================================================================
...
|search-content          |Mark the commits whose contents match
|=============================================================================
...
|=============================================================================
|edit                    |Open in editor
|edit-blob               |Open the file of the shown commit in editor
...
|main-match		|Marker of a commit whose contents match the search.
|=============================================================================

.Base diff colors
[frame="none",grid="none",cols="25<m,75<"]
|=============================================================================
|bdiff-add		|Marker of a commit which only exists on HEAD.
|bdiff-del		|Marker of a commit which is gone from HEAD.
|bdiff-move		|Marker of a reordered commit.
|bdiff-from		|Marker of where a reordered commit used to be.
|bdiff-change		|Marker of a modified commit.
|bdiff-same		|Marker of an unchanged commit.
|bdiff-move-change	|Marker of a commit both reordered and modified.
|bdiff-ctx		|Marker of a commit whose context changed, not its patch.
|bdiff-move-ctx		|Marker of a reordered commit whose context changed.
|bdiff-section		|Heading of a section of the base diff view.
|=============================================================================

.Base diff range colors
[frame="none",grid="none",cols="35<m,65<"]
|=============================================================================
|The range diff compares two patches, so each of its lines carries two
|markers: the outer one tells which patch the line belongs to, the inner one
|what that patch does to the file. The color follows the inner marker, the
|attribute the outer one.
|=============================================================================

[frame="none",grid="none",cols="35<m,65<"]
|=============================================================================
|bdiff-range-old-mark	|Marker of a line of the previous patch.
|bdiff-range-new-mark	|Marker of a line of the current patch.
|bdiff-range-both-context	|Context line both patches have.
|bdiff-range-both-del	|Deletion both patches make.
|bdiff-range-both-add	|Addition both patches make.
|bdiff-range-both-chunk	|Chunk header both patches have.
|bdiff-range-old-context	|Context line only the previous patch had.
|bdiff-range-old-del	|Deletion only the previous patch made.
|bdiff-range-old-add	|Addition only the previous patch made.
|bdiff-range-old-chunk	|Chunk header only the previous patch had.
|bdiff-range-new-context	|Context line only the current patch has.
|bdiff-range-new-del	|Deletion only the current patch makes.
|bdiff-range-new-add	|Addition only the current patch makes.
|bdiff-range-new-chunk	|Chunk header only the current patch has.
|bdiff-range-old-context-highlight	|What changed in a context line of the previous patch.
|bdiff-range-old-del-highlight	|What changed in a deletion of the previous patch.
|bdiff-range-old-add-highlight	|What changed in an addition of the previous patch.
|bdiff-range-new-context-highlight	|What changed in a context line of the current patch.
|bdiff-range-new-del-highlight	|What changed in a deletion of the current patch.
|bdiff-range-new-add-highlight	|What changed in an addition of the current patch.
|=============================================================================
