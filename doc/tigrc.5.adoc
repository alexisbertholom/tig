...
'diff-stat-group' (bool)::
	...
	Group the files in the diffstat under shared-prefix headers, to shorten
	the paths. The directories common to a run of files are printed once as a
	`[prefix/]` header (colored with `color diff-stat-header`) and elided from
	the files below it. Files are grouped only as deeply as needed for the
	diffstat to fit the view width, and never reordered, so jumping to a file
	diff keeps working. Off by default.
